...
class OracleLoggingTest extends TestCase {
	...
	function testLogInitAndTransactions() {
		...
		$this->assertEquals('ALTER SESSION SET NLS_TIMESTAMP_FORMAT = ', $ma->pdo->getLogger()->getEntries()[1]['sql']);
		$this->assertEquals('ALTER SESSION SET NLS_DATE_FORMAT = ', $ma->pdo->getLogger()->getEntries()[2]['sql']);
		$this->assertEquals('ALTER SESSION SET NLS_TIMESTAMP_TZ_FORMAT = ', $ma->pdo->getLogger()->getEntries()[3]['sql']);
		...
		$this->assertEquals('SET TRANSACTION ISOLATION LEVEL SERIALIZABLE', $ma->pdo->getLogger()->getEntries()[4]['sql']);
		...
		$this->assertEquals('commit', $ma->pdo->getLogger()->getEntries()[6]['type']);
		...
		$this->assertEquals('SET TRANSACTION ISOLATION LEVEL REPEATABLE READ', $ma->pdo->getLogger()->getEntries()[7]['sql']);
